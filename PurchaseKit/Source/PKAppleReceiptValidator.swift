...
public struct PKAppleReceiptValidator: PKReceiptValidator {

	public enum PKVerifyReceiptURLType: String {
		case production = "https://buy.itunes.apple.com/verifyReceipt"
		case sandbox = "https://sandbox.itunes.apple.com/verifyReceipt"
	}

	public init(service: PKVerifyReceiptURLType = .production) {
		self.service = service
	}

	private let service: PKVerifyReceiptURLType
	...
	public func validate(
		...
		// Remote task
		let task = URLSession.shared.dataTask(with: storeRequest as URLRequest) { data, _, error -> Swift.Void in
			...
			// cannot decode data
			guard let receiptInfo = try? JSONSerialization.jsonObject(with: data!, options: .mutableLeaves) as? PKReceipt ?? [:] else {
			...
			if let status = receiptInfo["status"] as? Int {
				...
				*/
				let receiptStatus = PKReceiptStatus(rawValue: status) ?? PKReceiptStatus.unknown
				if case .testReceipt = receiptStatus {
					let sandboxValidator = PKAppleReceiptValidator(service: .sandbox)
				...
				completion(.error(error: .receiptInvalid(receipt: receiptInfo, status: PKReceiptStatus.none)))
			}
		}
		task.resume()
	}
